...
import type { LiveSessionClient, QueryResult, StartParams } from '../../lib/liveSession'
...
export default function AskAiOverlay({ client, config, sessionStatus, theme, start, screenId }: { client: LiveSessionClient; config: any; sessionStatus: string; theme?: OverlayTheme; start: StartParams; screenId?: string }) {
  ...
  if (!open) return <button onClick={() => { ensure(key, cfg.historyLimit || 100); setOpen(key, true) }} className="absolute bottom-4 right-4 z-20 flex h-12 w-12 items-center justify-center text-white" style={{ backgroundColor: t.primary, borderRadius: t.radius, fontFamily: t.font }} title="Ask AI"><MessageCircle size={20} />{thread?.unread ? <span className="absolute -right-1 -top-1 min-w-4 rounded-full bg-red-600 px-1 text-[9px]">{thread.unread}</span> : null}</button>
  return (
    <div className="absolute bottom-4 right-4 z-20 flex max-h-[60vh] w-80 flex-col overflow-hidden border" style={{ backgroundColor: t.surface, color: t.foreground, borderColor: t.primary, borderRadius: t.radius, fontFamily: t.font }}>
      ...
      <div className="min-h-0 flex-1 space-y-2 overflow-y-auto p-3" aria-live="polite">
        {(thread?.messages || []).map(message => (
          <div key={message.id} className={message.role === 'user' ? 'ml-8 border px-2 py-1.5 text-xs' : 'mr-2 border px-2 py-1.5 text-xs'} style={{ borderColor: t.primary + '33', borderRadius: t.radius, opacity: message.status === 'error' ? 0.65 : 1 }}>
            <div className="mb-1 text-[9px] uppercase opacity-50">{message.role}</div>
            {message.status === 'pending' ? <Loader2 size={14} className="animate-spin" /> : editing === message.id ? <textarea autoFocus value={message.editedText ?? message.text} onChange={e => patch(key, message.id, { editedText: e.target.value }, cfg.historyLimit || 100)} onBlur={() => setEditing(null)} className="w-full border p-1 text-xs" /> : <div className="whitespace-pre-wrap">{message.editedText ?? message.text}</div>}
