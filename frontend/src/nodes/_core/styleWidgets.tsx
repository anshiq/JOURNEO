  ...
  return (
    <div className="space-y-1.5">
      ...
      <div className="flex items-center gap-2">
        <div className="relative h-8 w-10 shrink-0 overflow-hidden rounded-none border border-border/60">
...
export function SelectField({ label, value, onChange, options }: { label: string; value?: string; onChange: (v: any) => void; options: Array<{ value: string; label: string }> }) {
  return (
    <div className="space-y-1.5">
      <span className="text-[11px] font-medium text-muted-foreground">{label}</span>
      <select value={value ?? ''} onChange={e => { const v = e.target.value; if (v === '') onChange(undefined); else onChange(v) }} className="flex h-8 w-full rounded-none border border-input bg-background px-2 text-xs shadow-none focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
    ...
    <div className="space-y-1.5">
      ...
      <div className="grid grid-flow-col auto-cols-fr gap-1 rounded-none border bg-muted/30 p-0.5">
        {options.map(o => {
          ...
          return <button key={o.value} type="button" onClick={() => onChange(value === o.value ? undefined : o.value)} className={cn('rounded-none px-2 py-1 text-[11px] transition-colors', active ? 'bg-background text-foreground shadow-none' : 'text-muted-foreground hover:text-foreground')}>{o.label}</button>
        })}
      </div>
    </div>
  )
