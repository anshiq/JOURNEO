...
export const ImageStyleConfig: React.FC<StyleConfigProps> = ({ style, onChange, theme, config, onConfigChange }) => {
  ...
  const suggestAlt = () => {
    ...
    aiApi.post('/v1/vision/alt-text', { imageUrl: cfg.src })
      .then(res => { if (res.data?.alt) setCfg({ alt: res.data.alt }) })
      .finally(() => setSuggesting(false))
  }
  return <div className="space-y-4">{onConfigChange && <div className="space-y-3 rounded-none border bg-muted/20 p-3"><div className="flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-wider text-muted-foreground"><ImageIcon className="h-3.5 w-3.5" /> Image source</div><TextField label="Image URL" value={cfg.src || ''} onChange={v => setCfg({ src: v || '' })} placeholder="https://..." /><FileUploader value={cfg.src || ''} onChange={url => setCfg({ src: url })} kind="image" maxSizeMB={25} /><div className="flex items-end gap-2"><div className="flex-1"><TextField label="Alt text" value={cfg.alt || ''} onChange={v => setCfg({ alt: v || '' })} placeholder="Describe image" /></div><Button type="button" variant="outline" size="sm" disabled={!cfg.src || suggesting} onClick={suggestAlt}>{suggesting ? 'Suggesting…' : 'Suggest alt text'}</Button></div><TextField label="Aspect ratio" value={cfg.aspectRatio || ''} onChange={v => setCfg({ aspectRatio: v || '' })} placeholder="16/9" />{cfg.src && <img src={cfg.src} alt="" className="h-20 w-full object-cover rounded-none border" />}</div>}<TextField label="Radius" value={style.borderRadius} onChange={v => onChange('borderRadius', v)} placeholder="8px" /><SelectField label="Style" value={style.borderStyle} onChange={v => onChange('borderStyle', v)} options={[{value:'solid',label:'Solid'},{value:'dashed',label:'Dashed'},{value:'none',label:'None'}]} /><TextField label="Width" value={style.borderWidth} onChange={v => onChange('borderWidth', v)} placeholder="1px" />
