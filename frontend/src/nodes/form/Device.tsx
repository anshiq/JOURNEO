...
export const FormDevice: React.FC<DeviceProps> = ({ config, theme, value, error, onChange, onAdvance }) => {
  ...
  return (
    <div className="w-full min-w-0 max-w-full space-y-3 p-4 sm:space-y-4 sm:p-5" style={{ maxWidth: '100%', boxSizing: 'border-box' }}>
      ...
      {cfg.blockOwnsExit && <button onClick={e => { e.stopPropagation(); onAdvance('default', values) }} className="min-h-[48px] w-full min-w-0 max-w-full break-words py-2 text-sm font-medium text-white sm:text-base" style={{ backgroundColor: theme.primary, borderRadius: theme.radius }}>{cfg.submitLabel || 'Submit'}</button>}
    </div>
  )
}
