...
export const CountdownDevice: React.FC<DeviceProps> = ({ config, theme, onAdvance }) => {
  ...
  if (!cfg.endTime) return <div className="w-full min-w-0 max-w-full break-words py-6 text-center text-xs opacity-70 sm:text-sm">No end time</div>
  const days = Math.floor(remainingMs / 86400000)
  const hours = Math.floor((remainingMs % 86400000) / 3600000)
  const mins = Math.floor((remainingMs % 3600000) / 60000)
  const secs = Math.floor((remainingMs % 60000) / 1000)
  if (remainingMs === 0) return <div className="w-full min-w-0 max-w-full break-words py-6 text-center text-sm sm:text-base">{cfg.onExpire === 'message' && (cfg.expiredMessage || 'Offer expired')}</div>
  const size = cfg.size === 'sm' ? 'text-xl sm:text-2xl' : cfg.size === 'lg' ? 'text-3xl sm:text-4xl' : 'text-2xl sm:text-3xl'
  const units = [{ v: days, u: 'd' }, { v: hours, u: 'h' }, { v: mins, u: 'm' }, { v: secs, u: 's' }]
  return <div className="w-full min-w-0 max-w-full break-words py-6 text-center" style={{ maxWidth: '100%' }}>{cfg.showLabel !== false && <div className="mb-2 min-w-0 break-words px-2 text-xs opacity-70 sm:text-sm">{cfg.label || 'Offer ends in'}</div>}<div className={`flex min-w-0 max-w-full flex-wrap items-baseline justify-center gap-x-2 gap-y-1 font-mono tabular-nums ${size}`} aria-live="polite">{units.map(({ v, u }) => <span key={u} className="whitespace-nowrap">{v}{u}</span>)}</div></div>
}
