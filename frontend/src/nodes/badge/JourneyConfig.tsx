...
export const BadgeJourneyConfig: React.FC<JourneyConfigProps<Config>> = ({ config, onChange }) => {
  ...
  return <div className="space-y-3"><div className="space-y-1"><Label>Label</Label><Input {...form.register('label')} placeholder="Badge text, use {{fieldKey}} for dynamic values" /></div><div className="space-y-1"><Label>Variant</Label><Select value={form.watch('variant')} onValueChange={val => form.setValue('variant', val as any)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="solid">Solid</SelectItem><SelectItem value="soft">Soft</SelectItem><SelectItem value="outline">Outline</SelectItem></SelectContent></Select></div><div className="space-y-1"><Label>Color</Label><Input {...form.register('color')} placeholder="#4f46e5" /></div><label className="flex items-center gap-2 text-xs"><input type="checkbox" {...form.register('pulse')} /> Pulse animation</label><Button type="button" variant="outline" size="sm" onClick={() => onChange({ label: 'Badge', variant: 'soft' } as any)}>Reset</Button></div>
}
